...
if(trim($name_contact) == '') {
	echo '<div class="error_message">Por favor introduce tu nombre.</div>';
	exit();
} else if(trim($lastname_contact ) == '') {
	echo '<div class="error_message">Por favor introduce tu apellido.</div>';
	exit();
} else if(trim($email_contact) == '') {
	echo '<div class="error_message">Por favor introduce tu correo.</div>';
	exit();
} else if(!isEmail($email_contact)) {
	echo '<div class="error_message">Escribiste un correo no-valido, prueba de nuevo.</div>';
	exit();
	} else if(trim($phone_contact) == '') {
	echo '<div class="error_message">Por favor introduce un telefono veridico.</div>';
	exit();
} else if(!is_numeric($phone_contact)) {
	echo '<div class="error_message">El campo para tu numero de telefono solo pueden contener numeros.</div>';
	exit();
	...
	echo '<div class="error_message"> Por favor introduce la respuesta a la ecuación para validar que no eres un robot.</div>';
	exit();
} else if(trim($verify_contact) != '4') {
	echo '<div class="error_message">La respuesta a la ecuacion para validar que no eres un robot es incorecta.</div>';
	exit();
}
...
$usermessage = "Gracias por contactar a RitoImportz. Te responderemos a la brevedad!";
...
if(mail($address, $e_subject, $msg, $headers)) {
	...
	echo "<strong >Correo enviado con exito.</strong><br>";
	echo "Muchas gracias <strong>$name_contact</strong>,<br> tu mensaje ha sido enviado. Te contactaremos a la brevedad.";
	echo "</div>";
